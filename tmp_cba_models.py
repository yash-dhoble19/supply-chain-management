...
class Product(Base):
    ...
    )  # Raw Material ΓåÆ Work in Progress ΓåÆ Finished
...
class Forecast(Base):
    ...
    confidence_score = Column(Float)  # 0ΓÇô1 or %
...
class Order(Base):
    ...
    status = Column(
        String,
        default="PENDING"
    )  # PENDING ΓåÆ CONFIRMED ΓåÆ SHIPPED ΓåÆ DELIVERED
...
class Supplier(Base):
    ...
    reliability_score = Column(Float, default=95.0)  # 1ΓÇô10 scale
...
class PurchaseOrder(Base):
    ...
    status = Column(
        String,
        default="DRAFT"
    )  # DRAFT ΓåÆ APPROVED ΓåÆ IN_TRANSIT ΓåÆ RECEIVED
