...
const CATEGORIES = ["Semiconductors & Substrates", "Conductive Metals", "Insulating Material", "Soldering Materials", "Chemicals & Gases", "Rare Earths"];
...
const TableBodyRow = ({ 
    ...
    return (
      <tr className="border-b bg-yellow-50">
        ...
        <td className="px-4 py-3 font-semibold">
          {formatCurrency((productForm.current_stock || item.stock) * (productForm.unit_price || item.unit_price))}
        </td>
  ...
  return (
    <tr className="border-b hover:bg-slate-50 transition">
      ...
      <td className="px-4 py-3 font-semibold">{formatCurrency(item.total_value)}</td>
      <td className="px-4 py-3">
        ...
        <div className="space-y-6 p-4 sm:p-6 lg:p-8">
          ...
          <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="rounded-xl border border-surface-container-high p-5 bg-surface-container-lowest">
              ...
              <p className="mt-2 text-3xl font-bold">{summary ? formatCurrency(summary.total_value) : "--"}</p>
          ...
          <section className="rounded-xl border border-surface-container-high bg-surface-container-lowest overflow-hidden">
            <div className="p-5 border-b border-surface-container-high flex justify-between items-center">
              ...
              <span className="text-sm text-slate-500 bg-slate-100 px-3 py-1 rounded-full">{manufacturingGoods.length} items</span>
            </div>
            ...
            <div className="p-5 flex flex-col md:flex-row md:items-center md:justify-between gap-3 bg-surface-container-lowest border-b border-surface-container-high">
              ...
              <span className="text-sm text-slate-500">{manufacturingGoods.length} tasks currently</span>
            </div>
                          ...
                          {editingManufacturingId === item.id ? (
                            ...
                            <span className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold ${
                              ...
                              {item.status === 'Done' ? '✓ Done' :
                               item.status === 'Work In Progress' ? '⚙️ Work In Progress' :
                               item.status === 'Quality Check' ? '🔍 Quality Check' :
                               item.status === 'On Hold' ? '⏸️ On Hold' :
                               '📦 Pending'}
                            </span>
                          )}
                        </td>
